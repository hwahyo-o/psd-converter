...
- `figma-plugin/code.js`: UI를 포함한 단일 Figma Plugin API importer
...
3. 이 저장소의 `figma-plugin` 폴더에서 최신 `manifest.json`을 선택합니다. 같은 폴더에 최신 `code.js`가 있어야 합니다.
4. 이전에 등록한 `PSD 변환기`가 있다면 제거한 뒤 최신 manifest를 다시 Import합니다.
5. `PSD 변환기` 플러그인을 실행합니다.
6. 웹 앱에서 받은 payload zip을 압축 해제한 뒤 `document.json`을 선택하거나, 웹 앱에서 바로 받은 `document.json`을 선택합니다.
7. `Figma에 가져오기`를 클릭하면 현재 페이지에 LayerBridge 프레임이 생성됩니다.

## 플러그인 시작 구조

플러그인 UI는 `code.js`에 직접 포함되어 있습니다. 따라서 Development Plugin 실행 시 별도 `ui.html` 파일이나 `__html__` 빌드 전역값에 의존하지 않습니다.
